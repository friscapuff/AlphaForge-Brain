...
@indicator("dual_sma")
def dual_sma_indicator(df: pd.DataFrame, params: dict[str, int] | None = None) -> pd.DataFrame:  # legacy wrapper for tests
    ...
    if "close" not in df.columns:
        raise ValueError("DataFrame must contain 'close' column")
    out = df.copy()
    out[f"sma_short_{short_w}"] = out["close"].rolling(window=short_w, min_periods=short_w).mean()
    out[f"sma_long_{long_w}"] = out["close"].rolling(window=long_w, min_periods=long_w).mean()
    return out
