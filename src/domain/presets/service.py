...
class PresetService:
                ...
                try:
                    ...
                    if isinstance(data_all, dict):
                        for preset_id, rec in data_all.items():
                            if isinstance(rec, dict) and "name" in rec and "config" in rec:
                                created_at = rec.get("created_at") or datetime.now(timezone.utc).isoformat()
                                self._cache[preset_id] = PresetRecord(name=rec["name"], config=rec["config"], created_at=created_at)
        ...
        for p in self.root.glob("*.json"):
            try:
                ...
                if isinstance(data, dict) and "name" in data and "config" in data:
                    ...
                    self._cache[preset_id] = PresetRecord(name=data["name"], config=data["config"], created_at=created_at)
            except Exception:  # pragma: no cover - ignore corrupt files
                pass

    @staticmethod
    def _hash(name: str, config: dict[str, Any]) -> str:
        canonical = json.dumps({"name": name, "config": config}, sort_keys=True, separators=(",", ":")).encode()
    ...
    def create(self, name: str, config: dict[str, Any]) -> tuple[str, bool]:
        ...
        with self._lock:
            ...
            rec: PresetRecord = PresetRecord(name=name, config=config, created_at=datetime.now(timezone.utc).isoformat())
        ...
        if getattr(self, "_single_file", None):
            try:
                if self._single_file is not None:
                    self._single_file.write_text(json.dumps(self._cache, sort_keys=True), encoding="utf-8")
            except Exception:  # pragma: no cover
                pass
            return
        if new_ids:
...
class SQLitePresetService(PresetService):
    ...
    def __init__(self, db_path: Path | None = None):
        ...
        self._conn.execute("CREATE INDEX IF NOT EXISTS idx_presets_name ON presets(name)")
        self._conn.commit()
    ...
    def get(self, preset_id: str) -> PresetRecord | None:
        cur = self._conn.cursor()
        cur.execute("SELECT name, config_json, created_at FROM presets WHERE preset_id=?", (preset_id,))
        row = cur.fetchone()
        if not row:
            return None
        name, cfg_json, created_at = row
        return PresetRecord(name=name, config=json.loads(cfg_json), created_at=created_at)

    def list(self) -> list[dict[str, Any]]:
        cur = self._conn.cursor()
        cur.execute("SELECT preset_id, name, config_json, created_at FROM presets ORDER BY created_at ASC")
        rows = cur.fetchall()
        return [
            {"preset_id": pid, "name": name, "config": json.loads(cfg_json), "created_at": created_at}
    ...
    def __enter__(self) -> SQLitePresetService:  # pragma: no cover - rarely used in prod path
        return self

    from types import TracebackType
    def __exit__(self, exc_type: type[BaseException] | None, exc: BaseException | None, tb: TracebackType | None) -> None:  # pragma: no cover
        self.close()
...
def get_preset_service() -> PresetService:
    ...
    backend_mismatch = (
        (env_backend == "sqlite" and not isinstance(_service, SQLitePresetService)) or
        (env_backend != "sqlite" and isinstance(_service, SQLitePresetService))
    )
    path_mismatch = False
    if env_path and not isinstance(_service, SQLitePresetService):  # only relevant for file backend
