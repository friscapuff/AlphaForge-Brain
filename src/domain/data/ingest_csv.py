...
def _read_csv(path: Path) -> pd.DataFrame:
    ...
    if legacy_required.issubset(set(df.columns)):
        ...
        mapped["timestamp"] = pd.to_datetime(df["Date"], format="%m/%d/%Y", errors="coerce")
        ...
        mapped["close"] = pd.to_numeric(
            df["Close/Last"].astype(str).str.replace("$", "", regex=False).str.replace(",", "", regex=False),
            errors="coerce",
        )
        mapped["volume"] = pd.to_numeric(df["Volume"].astype(str).str.replace(",", "", regex=False), errors="coerce").astype("Int64")
        mapped = mapped.dropna(subset=["timestamp"])  # drop rows where date failed to parse
