...
async def create_preset(req: PresetCreate, svc: PresetService = Depends(svc_dep)) -> PresetResponse:  # noqa: B008 (FastAPI pattern)
...
async def list_presets(svc: PresetService = Depends(svc_dep)) -> PresetList:  # noqa: B008
    items = []
    for rec in svc.list():
        items.append(PresetResponse(preset_id=rec["preset_id"], name=rec["name"], config=rec["config"]))
    return PresetList(items=items)

@router.get("/presets/{preset_id}", response_model=PresetResponse)
async def get_preset(preset_id: str, svc: PresetService = Depends(svc_dep)) -> PresetResponse:  # noqa: B008
    rec = svc.get(preset_id)
    if not rec:
...
async def delete_preset(preset_id: str, svc: PresetService = Depends(svc_dep)) -> dict[str, str]:  # noqa: B008
