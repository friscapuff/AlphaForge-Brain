...
async def post_run(cfg: RunConfig, registry: InMemoryRunRegistry = Depends(_registry)) -> RunCreateResponse:  # T050
...
async def list_runs(registry: InMemoryRunRegistry = Depends(_registry)) -> RunListResponse:
    ...
    items = [RunListItem(run_hash=h, created_at=datetime.fromtimestamp(ts, tz=timezone.utc), status="SUCCEEDED") for h, ts in entries]
...
async def get_run_detail(
    ...
    include_anomalies: bool = Query(False, description="Include dataset anomaly counters when available"),
    ...
    if manifest_path.exists():
        try:
            ...
            for f in m.get("files", []):
                if isinstance(f, dict) and {"name","sha256","size"}.issubset(f.keys()):
                    artifacts.append(ArtifactDescriptor(name=f["name"], sha256=f["sha256"], size=int(f["size"])) )
    ...
    validation_summary = rec.get("validation_summary") if isinstance(rec.get("validation_summary"), dict) else None
...
async def cancel_run(run_hash: str, registry: InMemoryRunRegistry = Depends(_registry)) -> dict[str, str]:
    rec = registry.get(run_hash)
    if rec is None:
...
async def list_artifacts(run_hash: str, registry: InMemoryRunRegistry = Depends(_registry)) -> dict[str, Any]:
    rec = registry.get(run_hash)
    if rec is None:
        raise HTTPException(status_code=404, detail="run not found")
    base = resolve_artifact_root(None)
    files = artifact_index(run_hash, base_dir=base)
    return {"run_hash": run_hash, "files": files}


@router.get("/runs/{run_hash}/artifacts/{name}")
async def get_artifact(run_hash: str, name: str, registry: InMemoryRunRegistry = Depends(_registry)) -> Any:
    rec = registry.get(run_hash)
    if rec is None:
        raise HTTPException(status_code=404, detail="run not found")
    base = resolve_artifact_root(None)
