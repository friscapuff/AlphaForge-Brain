...
def install_error_handlers(app: FastAPI) -> None:
    ...
    async def domain_error_handler(request: Request, exc: DomainError) -> JSONResponse:  # pragma: no cover
        status = _CODE_STATUS.get(exc.code, 400)
        return JSONResponse(status_code=status, content=_error_envelope(exc))

    @app.exception_handler(Exception)
    async def unhandled_error_handler(request: Request, exc: Exception) -> JSONResponse:  # pragma: no cover
