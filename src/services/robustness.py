...
def robustness_score(
    ...
    p_component: float = (float(sum(p_values)) / float(len(p_values))) if p_values else 0.0
    stability = oos_consistency or 0.0
    # Simple weighted sum (0..1 domain expected)
    return 0.6 * p_component + 0.4 * stability

def compute_robustness_score(p_value: float, extreme_tail_ratio: float, oos_consistency_score: float) -> float:
    ...
    epsilon = 1e-9
    score: float = (evidence + epsilon) ** 0.4 * (extreme_tail_ratio + epsilon) ** 0.3 * (
        oos_consistency_score + epsilon
    ) ** 0.3
