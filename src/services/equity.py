...
def build_equity(trades: Iterable[Trade]) -> list[EquityBar]:
    ...
    for t in ordered:
        ...
        cash_flow = -t.price * t.quantity if t.side == TradeSide.BUY else t.price * t.quantity
        state.nav += cash_flow / 1_000_000  # scale factor placeholder to keep nav sensible
        if state.nav <= 0:
