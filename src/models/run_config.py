...
class RunConfig(BaseModelStrict):  # FR aggregate (references many FR groups)
    ...
    def deterministic_signature(self) -> str:
        ...
        for fs in sorted(self.features, key=lambda x: x.name):
            add("feature", f"{fs.name}:{fs.version}:{sorted(fs.inputs)}:{sorted(fs.params.items())}:{fs.shift_applied}")
        ...
        if self.walk_forward:
            ...
            add("wf_opt_grid", str(sorted((k, tuple(v)) for k, v in wf.optimization.param_grid.items())))
