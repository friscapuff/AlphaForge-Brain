...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", "-o", default=".mypy_snapshot.json", help="Snapshot JSON path")
    p.add_argument("--paths", nargs="*", default=["src"], help="Paths to check (default: src)")
    p.add_argument("--allow-nonzero", action="store_true", help="Do not exit non-zero if mypy fails (still record)")
    return p.parse_args()
...
def snapshot() -> int:
    ...
    for line in raw.splitlines():
        ...
        parts = line.split("::", 1)[0].split(":", 4) if line.count(":") >= 4 else line.split(":", 4)  # tolerate edge cases
        if len(parts) < 5:
            continue
