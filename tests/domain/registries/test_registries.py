...
def test_strategy_registry_decorator() -> None:
    ...
    class DualSMA:
        def run(self, features: object) -> list[object]:  # pragma: no cover - placeholder
            return []
...
def test_risk_registry_decorator() -> None:
    ...
    class FixedFraction:
        def size(self, equity: float, signal: float) -> float:  # pragma: no cover - placeholder
            return 1.0
...
def test_validation_registry_decorator() -> None:
    def permutation_test(state: object) -> dict[str, float]:  # pragma: no cover - placeholder
        return {"p_value": 0.5}
