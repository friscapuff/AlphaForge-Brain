...
def test_validation_merge_artifact(tmp_path: Path, monkeypatch):
    ...
    if detail_path.exists():
        ...
        expected_sections = {"permutation", "block_bootstrap", "monte_carlo_slippage", "walk_forward"}
        assert expected_sections.issubset(data.keys())
    else:
        raise AssertionError("validation_detail.json not produced yet (expected failure before T038 implementation)")
