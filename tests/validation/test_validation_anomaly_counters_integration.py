...
def test_validation_json_contains_anomaly_counters(tmp_path: Path, monkeypatch: Any) -> None:
    class DummyMeta:  # pragma: no cover - simple structure
        data_hash: str = "abc"
        calendar_id: str = "NASDAQ"
        anomaly_counters: ClassVar[dict[str, int]] = {"duplicates_dropped": 2, "unexpected_gaps": 1}
    ...
    assert "anomaly_counters" in summary, "anomaly_counters missing from validation summary"
