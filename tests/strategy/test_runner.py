...
def test_signals_deterministic_and_aligned(monkeypatch: MonkeyPatch) -> None:
    ...
    stats1 = RunnerStats()
    out1 = run_strategy(cfg, df, candle_hash="dummy", cache_root="./.tmp_cache", stats=stats1)
    ...
    stats2 = RunnerStats()
    out2 = run_strategy(cfg, df, candle_hash="dummy", cache_root="./.tmp_cache", stats=stats2)
...
def test_feature_reuse(monkeypatch: MonkeyPatch) -> None:
    ...
    assert "signal" in out.columns
    assert calls["count"] == 0, "Feature build should not have been invoked when features provided"
