...
def test_to_epoch_ms_ambiguous_fallback_nat() -> None:
    ...
    out_nat = to_epoch_ms(s, assume_tz="America/New_York", ambiguous="NaT", nonexistent="raise")
    assert len(out_nat) == 2
    assert out_nat.isna().all(), "Ambiguous times should map to NaT under ambiguous='NaT'"
...
def test_to_epoch_ms_nonexistent_spring_forward_shift_forward() -> None:
    ...
    out_shift = to_epoch_ms(s, assume_tz="America/New_York", ambiguous="raise", nonexistent="shift_forward")
...
def test_to_epoch_ms_nonexistent_spring_forward_nat() -> None:
    ...
    s = pd.Series(pd.to_datetime(dt_strings))
    out_nat = to_epoch_ms(s, assume_tz="America/New_York", ambiguous="raise", nonexistent="NaT")
    assert len(out_nat) == 2
    assert out_nat.isna().all(), "Nonexistent times should map to NaT under nonexistent='NaT'"


@pytest.mark.parametrize("ambiguous_strategy", ["NaT", "raise"])  # smoke parameterization
