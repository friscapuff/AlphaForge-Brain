...
def test_cost_application_order_and_components():
    ...
    assert math.isclose(breakdown.slippage, expected_slip_buy + expected_slip_sell, rel_tol=1e-9)
    ...
    assert math.isclose(breakdown.total(), breakdown.slippage + breakdown.spread + breakdown.fees + breakdown.borrow)
...
def test_parametrized_spread_vs_participation(use_spread: bool, spread_pct: float | None, participation: float | None):
    ...
    _, breakdown = apply_costs(trades, cfg)
    if use_spread:
        assert breakdown.spread > 0 and math.isclose(breakdown.spread, 20.0 * (spread_pct/2) * 10, rel_tol=1e-9)
    else:
        assert breakdown.spread > 0 and math.isclose(breakdown.spread, 20.0 * (participation/100.0) * 10, rel_tol=1e-9)
