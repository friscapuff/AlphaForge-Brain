...
def config(train: int, test: int, warmup: int = 0) -> WalkForwardConfig:
    return WalkForwardConfig(
        ...
        optimization=WalkForwardOptimizationConfig(enabled=True, param_grid={"a": [1, 2], "b": [10]}),
...
@pytest.mark.parametrize(
    "train,test,warmup,total", [
        ...
        (30, 5, 0, 90),
        (40, 10, 5, 160),
        (80, 20, 0, 95),  # total smaller than two full strides => expect 0 or 1 depending on strict coverage
    ]
)
