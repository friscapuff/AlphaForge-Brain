...
def test_run_detail_includes_new_dataset_fields_and_alias() -> None:
    ...
    assert set(["data_hash", "calendar_id", "validation_summary", "validation"]).issubset(detail.keys())
    ...
    assert "anomaly_counters" in summary_obj, "anomaly_counters missing when include_anomalies=true"
