...
def test_nvda_end_to_end_run_artifacts(nvda_canonical_slice, fast, slow, tmp_path):
    ...
    cfg2 = cfg.model_copy(update={"strategy": StrategySpec(name="dual_sma", params={"fast": fast, "slow": slow})})
    ...
    assert manifest2.get("chain_prev") == manifest1.get("manifest_hash"), "chain_prev does not reference first manifest hash"
    ...
    if "metrics" in summary and isinstance(summary["metrics"], dict):
        assert "total_return" in summary["metrics"], "total_return missing in summary metrics"
