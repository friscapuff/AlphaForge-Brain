...
def test_dual_sma_strategy_on_nvda_slice(nvda_canonical_slice: tuple[pd.DataFrame, DatasetMetadata]) -> None:
    ...
    first_date = datetime.fromtimestamp(int(slice_df.ts.iloc[0]) / 1000, tz=timezone.utc).date().isoformat()
    last_date = datetime.fromtimestamp(int(slice_df.ts.iloc[-1]) / 1000, tz=timezone.utc).date().isoformat()
    cfg = RunConfig(
    ...
    assert short_col in strat_df.columns, "Short SMA column missing from strategy output"
    ...
    assert (early_signals.isna() | (early_signals == 0)).all(), "Signals emitted before long SMA warmup complete"
    ...
    for i, sig in strat_df["signal"].items():
        ...
        if sig == 1:
            ...
            if not (math.isnan(prev_short) or math.isnan(prev_long)):
                assert not (prev_short > prev_long), "Long signal not at crossover (short already above long previous bar)"
        elif sig == -1:
            assert s_val < l_val, "Short signal without short<long at same bar"
            if not (math.isnan(prev_short) or math.isnan(prev_long)):
                assert not (prev_short < prev_long), "Short signal not at crossover (short already below long previous bar)"
    ...
    direct_df = StrategyRegistry.get("dual_sma")(features, {"short_window": fast, "long_window": slow})
