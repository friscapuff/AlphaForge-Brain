...
def _synth_frame() -> pd.DataFrame:
    ...
    for i in range(10):
        ...
        rows.append({
            ...
            "open": o, "high": h, "low": low, "close": c, "volume": vol, "position_size": pos, "signal": signal,
        })
        price = c
...
def test_execution_costs_reconciliation(tmp_path: Path, slip_bps: float, fee_bps: float, spread_pct: float) -> None:  # T013
    ...
    cfg_costs = RunConfig(
        ...
        risk=cfg_base.risk,
        execution=ExecutionSpec(slippage_bps=slip_bps, fee_bps=fee_bps, slippage_model={"model": "spread_pct", "params": {"spread_pct": spread_pct}}),
        validation=ValidationSpec(),
        symbol="TEST", timeframe="1m", start="2024-01-01", end="2024-01-02"
    )
    ...
    assert abs(avg_costs - avg_base) > 0, "Cost scenario did not change average execution prices"
    ...
    pd.testing.assert_frame_equal(fills_costs.reset_index(drop=True), fills_costs_2.reset_index(drop=True))
