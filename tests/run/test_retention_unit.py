...
class ArtifactType(str, Enum):  # Minimal enum stub for compatibility with test fixture structure
    JSON = "JSON"
...
def test_apply_retention(tmp_path: Path, keep: int | None, max_age: int | None, expected: int) -> None:
    manifest = _manifest(tmp_path)
    cfg = _cfg(keep, max_age)
    removed = apply_retention(cfg, manifest, now=datetime(2026, 1, 1, tzinfo=timezone.utc))
