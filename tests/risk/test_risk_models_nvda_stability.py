...
def _base_cfg(model: str, params: dict) -> RunConfig:
    return RunConfig(
        ...
        symbol="NVDA", timeframe="1m", start="2020-01-01", end="2020-12-31",
    )
...
def test_risk_models_size_positive_when_signal_and_price(nvda_slice):
    ...
    vol_cfg = _base_cfg("volatility_target", {"target_vol": 0.15, "lookback": 30, "base_fraction": 0.2})
    vol = apply_risk(vol_cfg, df, equity=equity)

    kelly_cfg = _base_cfg("kelly_fraction", {"p_win": 0.55, "payoff_ratio": 1.5, "base_fraction": 0.5})
...
def test_kelly_monotonic_with_probability(nvda_slice, p_win, payoff):
    ...
    base_fraction = 0.5
    cfg = _base_cfg("kelly_fraction", {"p_win": p_win, "payoff_ratio": payoff, "base_fraction": base_fraction})
    ...
    if p_win > 0.3:
        lower_cfg = _base_cfg("kelly_fraction", {"p_win": max(0.01, p_win - 0.2), "payoff_ratio": payoff, "base_fraction": base_fraction})
