...
def test_dual_sma_on_clean_dataset_duplicates_removed():
    ...
    raw = pd.DataFrame({"timestamp": ts_list, "open": open_, "high": high, "low": low, "close": close, "volume": volume})
    # Simulate cleaning (drop duplicate keep first, drop missing none here)
    cleaned = raw.drop_duplicates(subset=["timestamp"], keep="first").reset_index(drop=True)
...
def test_simple_sma_feature_columns_consistency():
