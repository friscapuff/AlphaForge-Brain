...
def _kelly_fraction_size(
    equity: float, price: float, p_win: float, payoff_ratio: float, base_fraction: float
) -> float:
...
def apply_risk(
    config: RunConfig, signals_df: pd.DataFrame, *, equity: float = 100_000.0
) -> pd.DataFrame:
    ...
    if model == "fixed_fraction":
        ...
        sizes = [
            (
                ...
                if not pd.isna(out["signal"].iloc[i])
                else 0.0
            )
        ...
        for i in range(len(out)):
            ...
            sizes.append(
                _volatility_target_size(equity, price, target_vol, rv, base_fraction)
            )
        out["position_size"] = sizes
        return out
        ...
        for i in range(len(out)):
            ...
            price = float(prices.iloc[i])
            sizes.append(
                _kelly_fraction_size(equity, price, p_win, payoff_ratio, base_fraction)
            )
        out["position_size"] = sizes
        return out
