...
from domain.data.ingest_nvda import (
    DatasetMetadata,
...
class DataSource(Protocol):  # pragma: no cover - interface only
    ...
    def load(self) -> tuple[pd.DataFrame, DatasetMetadata | GenericDatasetMetadata]:
        """Load (and possibly cache) the full canonical dataset for this (symbol,timeframe).

        Supports both legacy NVDA canonical ingestion and emerging generic CSV ingestion paths.
        """
...
class LocalCsvDataSource:
    ...
    def load(
        ...
        pd.DataFrame, DatasetMetadata | GenericDatasetMetadata
        ...
        try:
            ...
            if self.symbol.upper() == "NVDA":
                # load_canonical_dataset already returns a DataFrame; redundant reconstructions removed
                legacy_frame, legacy_meta = load_canonical_dataset(self.root)
                return legacy_frame, legacy_meta
            raise
        if (
            entry.symbol.upper() == "NVDA"
            and entry.path
            and entry.path.endswith("NVDA_5y.csv")
        ):
            legacy_frame, legacy_meta = load_canonical_dataset(self.root)
            return legacy_frame, legacy_meta
        if entry.path is None:
            raise ValueError("LocalCsvDataSource requires a path in registry entry")
        frame, meta = load_generic_csv(
            entry.symbol, entry.timeframe, Path(entry.path), entry.calendar_id
        )
        return frame, meta
