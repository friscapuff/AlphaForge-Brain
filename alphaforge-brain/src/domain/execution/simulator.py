...
def _apply_slippage_model(
    base_price: float,
    side: int,
    model_conf: Mapping[str, Any] | None,
    row: pd.Series,
    *,
    qty: float | None = None,
) -> float:
...
def _apply_costs(
    price: float, *, slippage_bps: float, fee_bps: float, side: int
) -> float:
...
def simulate(
    ...
    *,
    ...
    for i in range(len(sized_df)):
        ...
        if i > 0:
            ...
            if not pd.isna(prev.get("signal")) and prev.get("signal") != 0:
                ...
                exec_price = _apply_slippage_model(
                    exec_price,
                    direction,
                    slippage_model_conf,
                    row,
    ...
    if flatten_end and position != 0.0:
        ...
        exec_price = _apply_slippage_model(
            exec_price, direction, slippage_model_conf, last_row, qty=abs(position)
        )
        exec_price = _apply_costs(
            exec_price, slippage_bps=slippage_bps, fee_bps=fee_bps, side=direction
        )
        delta = -position
        notional = delta * exec_price
        cash -= notional
