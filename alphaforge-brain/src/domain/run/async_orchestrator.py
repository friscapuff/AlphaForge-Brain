...
class AsyncOrchestrator:
    ...
    async def run(self) -> dict[str, Any]:
        ...
        self._emit(
            "stage",
            {"stage": STAGES[0], "index": 1, "total": total, "progress": 1 / total},
        )
        ...
        base = (
            pd.Timestamp(self.config.start, tz=timezone.utc)
        ...
        self._emit(
            "stage",
            {"stage": STAGES[1], "index": 2, "total": total, "progress": 2 / total},
        )
        ...
        self._emit(
            "stage",
            {"stage": STAGES[2], "index": 3, "total": total, "progress": 3 / total},
        )
        await asyncio.sleep(0)
        if bool(self._cancel_requested):
            return self._cancel()
        ...
        self._emit(
            "stage",
            {"stage": STAGES[3], "index": 4, "total": total, "progress": 4 / total},
        )
        await asyncio.sleep(0)
        if bool(self._cancel_requested):
            return self._cancel()
        ...
        self._emit(
            "stage",
            {"stage": STAGES[4], "index": 5, "total": total, "progress": 5 / total},
        )
        await asyncio.sleep(0)
        if bool(self._cancel_requested):
            return self._cancel()
        ...
        self._emit(
            "stage",
            {"stage": STAGES[5], "index": 6, "total": total, "progress": 6 / total},
        )
        await asyncio.sleep(0)
        if bool(self._cancel_requested):
            return self._cancel()
        ...
        self._emit(
            "stage",
            {"stage": STAGES[7], "index": 8, "total": total, "progress": 8 / total},
        )
        await asyncio.sleep(0)
        if bool(self._cancel_requested):
            return self._cancel()
        ...
        self._emit(
            "stage",
            {"stage": STAGES[8], "index": 9, "total": total, "progress": 9 / total},
        )
        await asyncio.sleep(0)
        # COMPLETE
        result: dict[str, Any] = {
