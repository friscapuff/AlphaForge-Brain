...
class Orchestrator:
    ...
    def _emit(
        self, state: OrchestratorState, payload: dict[str, Any] | None = None
    ) -> None:
    ...
    def run(self) -> dict[str, Any]:
        ...
        try:
            ...
            candles: pd.DataFrame | None = getattr(
                self.config, "_injected_candles", None
            )
            if candles is None:
                ...
                try:
                    ...
                    if (
                        self.config.symbol.upper() == "NVDA"
                        and self.config.timeframe == "1d"
                    ):
                        register_dataset(
                            DatasetEntry(
                                ...
                                provider="local_csv",
                                path="data/NVDA_5y.csv",
                                calendar_id="NASDAQ",
                            )
                        )
                        entry = get_dataset(self.config.symbol, self.config.timeframe)
                    elif self.config.symbol.upper() in {
                        "TEST",
                        "ASYNC",
                        "IDEMP",
                        "CHAIN",
                ...
                if candles is not None and entry.provider == "in_memory":
                    ...
                    try:
                        ...
                        start_ts = (
                            pd.Timestamp(self.config.start).tz_localize("UTC").value
                            // 1_000_000
                        )
                        end_ts = (
                            pd.Timestamp(self.config.end).tz_localize("UTC").value
                            // 1_000_000
                        )
                        ...
                        candles = slice_dataset(
                            entry.symbol, entry.timeframe, start_ts, end_ts
                        )
                else:  # pragma: no cover - future provider path
                    raise NotImplementedError(
                        f"Provider not implemented: {entry.provider}"
                    )
            ...
            if self.config.strategy.name == "dual_sma":
                strat_fast = int(
                    self.config.strategy.params.get(
                        "fast", self.config.strategy.params.get("short_window", 10)
                    )
                )
                strat_slow = int(
                    self.config.strategy.params.get(
                        "slow", self.config.strategy.params.get("long_window", 50)
                    )
                )
            ...
            if len(sma_windows) >= 2:
                ...
                candles[f"sma_short_{short_w}"] = (
                    candles["close"].rolling(window=short_w, min_periods=short_w).mean()
                )
            ...
            val_cfg = (
                self.config.validation.model_dump(mode="python")
    ...
    def _transition(
        self, new_state: OrchestratorState, payload: dict[str, Any] | None = None
    ) -> None:
