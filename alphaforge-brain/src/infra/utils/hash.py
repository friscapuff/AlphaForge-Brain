...
)  # relative import avoids attr-defined ambiguity for mypy
...
def canonical_json(obj: Any) -> str:
    ...
    def _float_truncate(
        o: Any,
    ) -> Any:  # pragma: no cover - deterministic formatting focus
        ...
        if isinstance(x, datetime):  # pragma: no cover - simple conversion
            if x.tzinfo is not None:
                try:
                    ...
                    x_utc = x
                return (
                    x_utc.replace(tzinfo=timezone.utc)
                    .isoformat()
                    .replace("+00:00", "Z")
                )
            return x.isoformat()
    ...
    return json.dumps(
        transformed, separators=CANONICAL_SEPARATORS, sort_keys=True, ensure_ascii=False
    )
