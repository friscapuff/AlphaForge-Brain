...
class TradeRepository:
    ...
    def list_for_run(self, run_hash: str) -> Sequence[m.Trade]:
        return self.s.execute(select(m.Trade).where(m.Trade.run_hash == run_hash)).scalars().all()
...
class EquityRepository:
    ...
    def list_for_run(self, run_hash: str) -> Sequence[m.Equity]:
        return self.s.execute(select(m.Equity).where(m.Equity.run_hash == run_hash)).scalars().all()
...
class FeatureRepository:
    ...
    def list_for_run(self, run_hash: str) -> Sequence[m.Feature]:
        return self.s.execute(select(m.Feature).where(m.Feature.run_hash == run_hash)).scalars().all()
...
class ValidationRepository:
    ...
    def list_for_run(self, run_hash: str) -> Sequence[m.Validation]:
        return (
            self.s.execute(select(m.Validation).where(m.Validation.run_hash == run_hash))
            .scalars()
            .all()
        )
...
class AuditLogRepository:
    ...
    def list_for_run(self, run_hash: str) -> Sequence[m.AuditLog]:
        return self.s.execute(select(m.AuditLog).where(m.AuditLog.run_hash == run_hash)).scalars().all()
