...
async def _stream(
    request: Request, run_hash: str, last_event_id: int | None
    ...
    if registry is None:
        ...
        for route in app.routes:
            if getattr(route, "path", "").startswith("/runs/{run_hash}") and hasattr(
                route, "endpoint"
            ):
                fn = route.endpoint
                if getattr(fn, "__name__", "") == "get_run" and getattr(
                    fn, "__closure__", None
                ):
...
async def run_events(
    request: Request, run_hash: str, after_id: int | None = None
) -> Response:
    ...
    lei_hdr = request.headers.get("last-event-id") or request.headers.get(
        "Last-Event-ID"
    )
...
async def _heartbeat_stream(
    interval_sec: int, request: Request
) -> AsyncGenerator[bytes, None]:  # pragma: no cover (timing logic)
    import asyncio
    ...
    async def generator() -> AsyncGenerator[bytes, None]:  # incremental
        ...
        while True:
            ...
            if rec is None:
                ...
                yield _event(
                    (last_event_id or 0) + 1, "error", {"code": "NOT_FOUND"}
                ).encode()
                break
            # New events?
            ...
            if new_events:
                ...
                # heartbeat
                yield _event(
                    (last_event_id or 0), "heartbeat", {"status": rec.get("status")}
                ).encode()
