...
def test_metrics_index_continuity_after_row_drops() -> None:
    ...
    assert curve_clean[
        "timestamp"
    ].is_monotonic_increasing, "Timestamps not strictly increasing after row drops"
    assert (
        not curve_clean["equity"].isna().any()
    ), "Equity contains NaNs after row drops"
    assert (
        not curve_clean["return"].isna().any()
    ), "Return contains NaNs after row drops"
