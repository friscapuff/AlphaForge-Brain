    ...
    df = pd.DataFrame(
        {
            ...
            "open": prices,
            "signal": [np.nan]
            + [1] * (n - 1),  # treat every bar after first as active signal
        }
    )
    return df
...
def test_volatility_target_sizes_scale_inverse_with_realized_vol() -> None:
    ...
    cfg = _base_cfg(
        RiskSpec(
            model="volatility_target",
            params={"target_vol": 0.15, "lookback": 5, "base_fraction": 0.2},
        )
    )
    ...
    for i in range(lookback + 1, len(rolling_std) - 1):
        ...
        for j in range(i + 1, len(rolling_std)):
            if (
                rolling_std.iloc[j] < rv_i and rolling_std.iloc[j] < 1e8
            ):  # ensure both are real (not sentinel)
                idx1 = i
                idx2 = j
                break
        if idx1 is not None:
...
def test_kelly_fraction_respects_probability_and_payoff() -> None:
    ...
    cfg2 = _base_cfg(
        RiskSpec(
            model="kelly_fraction",
            params={"p_win": 0.2, "payoff_ratio": 1.2, "base_fraction": 0.5},
        )
    )
    ...
    cfg3 = _base_cfg(
        RiskSpec(
            model="kelly_fraction",
            params={"p_win": 0.4, "payoff_ratio": 3.0, "base_fraction": 0.5},
        )
    )
