...
def _base_config() -> RunConfig:
    return RunConfig(
        indicators=[
            IndicatorSpec(name="dual_sma", params={"short_window": 2, "long_window": 4})
        ],
...
def build_sized_frame(
    signals: list[int] | list[float], position_size: int | float = 10, start: int = 0
) -> pd.DataFrame:
...
def test_t_plus_one_fill_rule() -> None:
    ...
    sized = build_sized_frame(
        [1, 0, 0]
    )  # signals for bars 1-3 lead to potential fills at bars 2-4
...
def test_cost_application_fee_and_slippage_direction() -> None:
    ...
    sized = build_sized_frame(
        [1, 0]
    )  # signal followed by neutral to allow T+1 execution
...
def test_empty_input_returns_empty_frames() -> None:
    cfg = _base_config()
    empty = pd.DataFrame(
        columns=["timestamp", "open", "close", "signal", "position_size"]
    ).iloc[0:0]
...
def _config(
    fast: int = 3, slow: int = 6, fee_bps: float = 5.0, slippage_bps: float = 10.0
) -> RunConfig:
    return RunConfig(
        indicators=[
        ...
        strategy=StrategySpec(
            name="dual_sma", params={"short_window": fast, "long_window": slow}
        ),
...
def test_t_plus_one_fill_and_skip_last_bar_signal() -> None:
    ...
    if not fills_df.empty:
        ...
        assert last_ts not in set(
            fills_df["timestamp"]
        )  # ensure last-bar signals skipped
...
def test_zero_volume_skips_fill_and_flatten_end_flag() -> None:
    ...
    if sized.shape[0] > 10:
        ...
        assert zero_ts not in set(
            fills_df["timestamp"]
        ), "Fill should be skipped on zero-volume bar when skip_zero_volume=True"
...
def test_flatten_end_realizes_final_position() -> None:
    ...
    last_idx = sized.index[
        -2
    ]  # signal on penultimate bar executes on last, leaving position potentially open
