...
@pytest.fixture
def sized_df_with_zero_volume(
    nvda_canonical_slice: tuple[pd.DataFrame, Any],
) -> tuple[pd.DataFrame, RunConfig]:  # returns sized frame and config
    ...
    df["signal"] = pd.Series([math.nan] + [1, -1] * (len(df) // 2), index=df.index)[
        : len(df)
    ]
    ...
    cfg = RunConfig(
        ...
        strategy=StrategySpec(
            name="dual_sma", params={"short_window": 3, "long_window": 5}
        ),
...
def test_skip_zero_volume_holds_position(
    sized_df_with_zero_volume: tuple[pd.DataFrame, RunConfig],
) -> None:
    sized, cfg = sized_df_with_zero_volume
    fills_skip, positions_skip = simulate(
        cfg, sized, initial_cash=50_000, skip_zero_volume=True
    )
    ...
    fills_all, positions_all = simulate(
        cfg, sized, initial_cash=50_000, skip_zero_volume=False
    )
