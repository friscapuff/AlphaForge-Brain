...
def _load_snapshot() -> dict[str, Any]:
    if not SNAPSHOT_PATH.exists():
        pytest.skip(
            "OpenAPI snapshot missing - generate snapshot before running diff test."
        )
    return json.loads(SNAPSHOT_PATH.read_text("utf-8"))


def _compare_dict_keys(
    old: dict[str, Any],
    new: dict[str, Any],
    prefix: str,
    removed: set[str],
...
def _walk_schema(old: Any, new: Any, path: str, removed: set[str], added: set[str]):
    ...
    if isinstance(old, dict) and isinstance(new, dict):
        ...
        if old.__class__ is not new.__class__:
            removed.add(
                f"{path[:-1]}:type_changed:{type(old).__name__}->{type(new).__name__}"
            )
...
def test_openapi_additive_diff():
    ...
    assert current.get("info", {}).get("version") == snapshot.get("info", {}).get(
        "version"
    ), "Version drift - bump snapshot alongside package version bump."
    ...
    for section in ("paths", "components"):
        if section in snapshot and section in current:
            _walk_schema(
                snapshot[section], current[section], f"{section}.", removed, added
            )
    ...
    if removed:
        ...
        pytest.fail(
            f"Breaking OpenAPI change(s) detected (removed/changed keys):\n{missing}\nTotal removed: {len(removed)}"
        )
