...
def _config() -> RunConfig:
    return RunConfig(
        ...
        strategy=StrategySpec(
            name="dual_sma", params={"short_window": 4, "long_window": 10}
        ),
...
def test_permutation_p_value_and_determinism() -> None:
    ...
    result = permutation.permutation_test(
        trades_df=trades, positions_df=positions, n=50, seed=123
    )
    assert {"p_value", "observed_mean", "null_mean", "null_std", "samples"}.issubset(
        result.keys()
    )
    assert 0 <= result["p_value"] <= 1
    # Determinism
    result2 = permutation.permutation_test(
        trades_df=trades, positions_df=positions, n=50, seed=123
    )
