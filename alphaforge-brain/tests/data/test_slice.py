...
class DummyProvider:
    ...
    ) -> pd.DataFrame:  # signature compatibility
        self.load_calls += 1
        return self.df
...
def test_slice_basic_window(
    provider: DummyProvider, raw_df: pd.DataFrame, cache_dir: Path
) -> None:
...
def test_slice_cache_reuse(
    ...
    out1 = slice_mod.get_candles_slice(
        "TEST", start, end, provider="dummy", cache_dir=cache_dir
    )
    out2 = slice_mod.get_candles_slice(
        "TEST", start, end, provider="dummy", cache_dir=cache_dir
    )

    assert (
        provider.load_calls == 1
    ), "Provider raw load should happen once due to cache + memoization"
    assert (
        cache_metrics.get().hits >= baseline_hits + 1
    ), "Expected at least one cache hit on second call"
    pd.testing.assert_frame_equal(out1, out2)


def test_slice_out_of_range_returns_empty(
    provider: DummyProvider, raw_df: pd.DataFrame, cache_dir: Path
) -> None:
    from domain.data import slice as slice_mod
...
def test_slice_memoization(
    ...
    assert (
        provider.load_calls == 1
    ), "Memoization should prevent second provider load in-process"
