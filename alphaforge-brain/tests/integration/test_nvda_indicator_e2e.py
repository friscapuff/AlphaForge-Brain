...
def test_nvda_slice_indicators_end_to_end(
    nvda_canonical_slice: tuple[Any, Any]
) -> None:
    ...
    dual_df = dual_sma_indicator(
        slice_df, params={"short_window": 5, "long_window": 20}
    )

    # Assertions
    sma_col = sma.feature_columns()[0]
    assert (
        sma_col in featured.columns
    ), "Simple SMA feature missing in feature engine output"
    ...
    if "zero_volume" in slice_df.columns:
        ...
        pd.testing.assert_series_equal(
            slice_df["zero_volume"], featured["zero_volume"], check_names=True
        )
