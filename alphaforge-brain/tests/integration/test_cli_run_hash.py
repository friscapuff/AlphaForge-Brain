...
def _make_config(seed: int = 1) -> RunConfig:
    ...
    exec_cfg = ExecutionConfig(
        fill_policy=FillPolicy.NEXT_BAR_OPEN, lot_size=1, rounding_mode=RoundingMode.ROUND
    )
    ...
    val_cfg = ValidationConfig(permutation_trials=10, seed=seed, caution_p_threshold=0.1)
    fs = FeatureSpec(name="ma", version="1", inputs=["close"], params={"w": 5}, shift_applied=1)
    return RunConfig(
        dataset=ds,
        features=[fs],
        strategy=strat,
...
def test_cli_run_hash_matches_service(tmp_path: Path):
    ...
    runner = CliRunner()
    res = runner.invoke(run_hash_cli, ["--config", str(cfg_path), str(p1), str(p2), "--quiet"])
...
def test_cli_run_hash_order_independent(tmp_path: Path):
    ...
    runner = CliRunner()
    res1 = runner.invoke(run_hash_cli, ["--config", str(cfg_path), str(p1), str(p2), "--quiet"])
    res2 = runner.invoke(run_hash_cli, ["--config", str(cfg_path), str(p2), str(p1), "--quiet"])
