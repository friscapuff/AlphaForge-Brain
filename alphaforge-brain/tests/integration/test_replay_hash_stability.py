...
def test_replay_hash_stability(tmp_path: Path, fast: int, slow: int) -> None:
    ...
    run_hash1, record1, created1 = create_or_get(
        cfg, registry, seed=seed, artifacts_base=artifacts_dir
    )
    ...
    run_hash2, record2, created2 = create_or_get(
        cfg, registry, seed=seed, artifacts_base=artifacts_dir
    )
    ...
    if "trades.parquet" in hashes1 or "trades.parquet" in hashes2:
        ...
        assert (
            hashes1["trades.parquet"] == hashes2["trades.parquet"]
        ), "Hash drift for trades.parquet"
