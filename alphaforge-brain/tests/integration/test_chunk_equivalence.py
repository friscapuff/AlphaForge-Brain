    ...
    df = pd.DataFrame(
        {
            ...
            "close": np.linspace(100, 200, n) + np.sin(np.arange(n) / 50.0),
...
def test_chunked_equivalence_sma(tmp_path):
    ...
    for cs in (256, 512, 1024):
        chunked = build_features(df, use_cache=False, chunk_size=cs, overlap=49)
        # Allow tiny floating differences due to chunk boundaries and FP summation order
        pd.testing.assert_frame_equal(
            mono, chunked, check_dtype=False, check_exact=False, rtol=1e-12, atol=1e-12
        )
