...
def _run_config() -> RunConfig:
    return RunConfig(
        indicators=[
            IndicatorSpec(name="dual_sma", params={"short_window": 3, "long_window": 6})
        ],
        strategy=StrategySpec(
            name="dual_sma", params={"short_window": 3, "long_window": 6}
        ),
...
def test_sse_events_and_resume() -> None:
    ...
    resp2 = client.get(
        f"/runs/{run_hash}/events", headers={"Last-Event-ID": str(last_id)}
    )
...
def test_sse_stream_incremental_and_etag() -> None:
    ...
    rstream = client.get(
        f"/runs/{run_hash}/events/stream", headers={"Accept": "text/event-stream"}
    )
